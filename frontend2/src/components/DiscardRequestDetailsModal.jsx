...
const DiscardRequestDetailsModal = ({ open, onClose, request, onApprove, onReject }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 relative font-sans">
        ...
        {/* Header */}
        <div className="pb-2">
          <h2 className="text-2xl font-bold text-gray-900">Discard Request Details</h2>
          <div className="text-sm text-gray-500 mt-1 mb-4">View details for discard request #{request.id}</div>
        </div>
        {/* Details Grid */}
        <div className="grid grid-cols-2 gap-x-8 gap-y-3 mb-6">
          ...
          <div>
            ...
            <div className="text-xs font-semibold text-gray-500 uppercase mb-1">Date Requested</div>
            <div className="text-base text-gray-800 mb-3">{request.date_requested ? new Date(request.date_requested).toLocaleDateString() : 'N/A'}</div>
            <div className="text-xs font-semibold text-gray-500 uppercase mb-1">Reason</div>
            <div className="text-base text-gray-800 mb-3">{request.reason || 'N/A'}</div>
          </div>
          {/* Right column */}
          <div>
            <div className="text-xs font-semibold text-gray-500 uppercase mb-1">Requested By</div>
            <div className="text-base text-gray-800 mb-3">{request.requested_by?.name || 'N/A'}</div>
            ...
            <div className="text-base text-gray-800 mb-3">{request.notes || 'N/A'}</div>
          </div>
        </div>
        {/* Buttons */}
...
export default DiscardRequestDetailsModal; 